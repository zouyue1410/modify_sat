...
class GraphConv(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fcv_Q_pos=nn.Sequential(
            nn.Linear(input_size , output_size),
            nn.ReLU(),
        )
        self.fcv_Q_neg = nn.Sequential(
            nn.Linear(input_size , output_size),
            nn.ReLU(),
        )
        self.fcv_K_pos = nn.Sequential(
            nn.Linear(input_size , output_size),
            nn.ReLU(),
        )
        self.fcv_K_neg = nn.Sequential(
            nn.Linear(input_size , output_size),
            nn.ReLU(),
        )
        self.fvc_Q_pos = nn.Sequential(
            nn.Linear(input_size , output_size),
            nn.ReLU(),
        )
        self.fvc_Q_neg = nn.Sequential(
            nn.Linear(input_size , output_size),
            nn.ReLU(),
        )
        self.fvc_K_pos = nn.Sequential(
            nn.Linear(input_size , output_size),
            nn.ReLU(),
        )
        self.fvc_K_neg = nn.Sequential(
            nn.Linear(input_size , output_size),
            nn.ReLU(),
        )
    ...
    def message(self, h, data):
        ...
        hcv = F.softmax(torch.spmm(hc.clone(), hv.clone().transpose(0, 1)))
        catt = torch.mul(cadj.to_dense(),torch.cat([hcv,hcv],dim=1))
        mc = torch.spmm(catt, torch.cat([self.fmc_pos(hv.clone()), self.fmc_neg(hv.clone())], dim=0))
        '''
        # 第二次修改KQV
        # 求mv
        cv_Q_pos=self.fcv_Q_pos(hv.clone())
        cv_K_pos=self.fcv_K_pos(hc.clone())
        cv_Q_neg = self.fcv_Q_neg(hv.clone())
        cv_K_neg = self.fcv_K_neg(hc.clone())
        cv_V_pos=self.fmv_pos(hc.clone())
        cv_V_neg=self.fmv_neg(hc.clone())
        cv_QtimesK_pos=torch.spmm(cv_Q_pos,cv_K_pos.transpose(0,1))#30*73
        cv_QtimesK_neg = torch.spmm(cv_Q_neg, cv_K_neg.transpose(0, 1))#30*73
        cv_norm_weights=torch.mul(vadj.to_dense(),torch.cat([cv_QtimesK_pos,cv_QtimesK_neg],dim=1))
        mv=torch.spmm(cv_norm_weights,torch.cat([cv_V_pos,cv_V_neg], dim=0))# 30*146    146*3

        # 求mc
        vc_Q_pos = self.fvc_Q_pos(hc.clone())
        vc_K_pos = self.fvc_K_pos(hv.clone())
        vc_Q_neg = self.fvc_Q_neg(hc.clone())
        vc_K_neg = self.fvc_K_neg(hv.clone())
        vc_V_pos = self.fmc_pos(hv.clone())
        vc_V_neg = self.fmc_neg(hv.clone())
        vc_QtimesK_pos = torch.spmm(vc_Q_pos, vc_K_pos.transpose(0, 1))  # 30*73
        vc_QtimesK_neg = torch.spmm(vc_Q_neg, vc_K_neg.transpose(0, 1))  # 30*73
        vc_norm_weights = torch.mul(cadj.to_dense(), torch.cat([vc_QtimesK_pos, vc_QtimesK_neg], dim=1))
        mc = torch.spmm(vc_norm_weights, torch.cat([vc_V_pos, vc_V_neg], dim=0))  # 30*146    146*3
        # 原模型消息传递
